...
Freeverb::Freeverb()
{
	...
	setUniqueID('qbq');	// identify - CHANGE THIS TO MAKE YOUR OWN!!!
	canMono();				// makes sense to feed both inputs with the same signal
	canProcessReplacing();	// supports both accumulating and replacing output
	strcpy(programName, "Default");	// default program name
#endif
	model.setroomsize(0.4f);
	model.setdamp(0.6f);
	model.setwet(0.25f);
	model.setdry(0.75f);
	//model.mode(0.0f);
...
{
    strcpy (text, "qbq"); // Change this to what you want!!
    return true;
}
...
void Freeverb::getParameterDisplay(long index, char *text)
{
	...
	case KRoomSize:
		...
		sprintf(text, "%f", model.getroomsize()*scaleroom+offsetroom);
		break;
	case KDamp:
		//long2string((long)(model.getdamp()*100), text);
		sprintf(text, "%ld", (long)(model.getdamp()*100));
		break;
	case KWet:
		//dB2string(model.getwet()*scalewet,text);
		sprintf(text, "%f", model.getwet()*scalewet);
		break;
	case KDry:
	...
	case KWidth:
		...
		sprintf(text, "%ld", (long)(model.getwidth()*100));
		break;
	}
}
...
{
	//printf("in freeverb %s: samples=%ld\n", __func__, sampleFrames);
	model.processmix(*inputs,(*inputs) + 1, *outputs, (*outputs) + 1,sampleFrames,1);
}
